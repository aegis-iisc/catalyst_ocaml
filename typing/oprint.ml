...
let rec print_out_module_type ppf =
  ...
  | Omty_functor (name, mty_arg, mty_res) ->
      let arg = match mty_arg with 
          | Some mty_a -> mty_a
          | None  -> Omty_abstract in
      fprintf ppf "@[<2>functor@ (%s : %a) ->@ %a@]" name
          print_out_module_type arg print_out_module_type mty_res
  ...
  *) | Osig_modtype (name, Omty_abstract) ->
  ...
  | Osig_type(td, rs) ->
        print_out_type_decl
          ...
          ppf td
  | Osig_value ovdecl ->
    let (name, ty, prims) = (ovdecl.oval_name, ovdecl.oval_type, ovdecl.oval_prims) in  
...
and print_out_type_decl kwd ppf  oty = 

let (name, args, ty, priv, constraints) = (oty.otype_name,
    oty.otype_params,
    oty.otype_type,
    oty.otype_private,
    oty.otype_cstrs) in 
...
and print_out_constr ppf (name, tyl, otyo) =
  match tyl with
    [] -> fprintf ppf "%s" name
  | _ ->
