		...
		| x :: xs -> 
			match x with 
				...
				| Int i -> (*parse internal node*) 
						parse_node root l_r ls



and parse_node root l_r ls = 
 		let () = Printf.printf "%s" "\n parse_node " in 
			...
			| x :: xs ->
				match x with 		 
				 | Int i -> 
					 ...
					 match l_r with 
					 | LT -> 
					 	let () = check (node <= root) in 
					 		 let btl_pair = parse_bst node LT xs in 
						 let rem = projr btl_pair in 
						 let btr_pair = parse_bst node GT rem in
...
let () = 
      ...
      let parsedList  = parse_bst 100 NC src  in 
      let () = Printf.printf "%s" ("\n test1 ") in 
