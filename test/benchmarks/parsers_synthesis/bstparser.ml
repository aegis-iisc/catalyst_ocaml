...
let rec parse_bstl root l = 
	let () = Printf.printf "%s" " \n parse_bstl " in
	match l with 
		| [] -> raise (TestExp "empty node :: bstl")	
		| x :: xs ->
			match x with 
			 | Char s -> parse_leaf l 	
			 | Int i -> 
			 	let () = check (i <= root) in 
			 	let new_root =  i in 
			 	let left_pair = parse_bstl new_root xs in 
			 	let rem1 = projr left_pair in 
			 	let right_pair = parse_bstr new_root rem1 in 
			 	let rem2 = projr right_pair in 
			 	let res = Node (new_root, projl left_pair, projl right_pair) in 
			 	Pair (res, rem2)
			


and parse_bstr root l = 
	let () = Printf.printf "%s" " \n parse_bstr " in
	match l with 
		| [] -> raise (TestExp "empty node :: bstl")	
		| x :: xs ->
		...
		match ls with
			...
			| x :: xs ->
				match x with 		 
				 | Int i -> 
					 let node = i in 
					 let btl_pair = parse_bstl node xs in 
					 let rem = projr btl_pair in 
					 let btr_pair = parse_bstr node rem in
...
let () = 
       ...
       *)let src =  [ Int 1 ; Char "<" ; Int 0 ; Char ">" ; Int 2 ; Char "<" ; Int 0 ; Char ">" ; Char "<" ; Int 0 ; Char ">" ] in 
