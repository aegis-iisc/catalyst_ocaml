(* 
	...
	different vc_tests
	1. parse_spaces (json_with_ws.ml/spec) (fails on changing the spec from C= to =)
	2. parse_token (json_ws_instantiated)
	3. parse_number_ref (json_num_with_token)
	4. parse_colon_ref less precise (json_colon_imprecise)
	5. parse_colon_ref more_precise (json_colon_precise)
	6. failing_of 3 and 5 ()
