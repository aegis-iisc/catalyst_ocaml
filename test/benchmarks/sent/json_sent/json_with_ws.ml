...
(* let parse_token (fwsp) (lst) (inparser) = 
	...
	(* let rem = projr res in
	let two = 2 in  
	let jsdummy = Num two in 
	let resdummy = ParseRes (jsdummy, rem) in  
	resdummy 
	 *)
	 res  
...
let () = 
	(* {a : 0}*)
	let input = [32;32;123;32;97; 58 ;49;125] in 
	let res = parse_spaces input in
 	let () = Printf.printf "%s" ("\n test1 ") in 
    let () = Printf.printf "%s" ("\ninput list "^(intlist_to_string input)) in 
    (* let () = Printf.printf "%s" ("\n output "^(output_to_string res)) in *) 
    let () = Printf.printf "%s" ("\noutput list "^(intlist_to_string res)) in 
