...
parse_token :  (l -> {vfws | (Rmem(vfws) C= Rmem(l)) /\ (Rmem(l) - Rmem(vfws) = {(32)}) \/ (Rmem(l) - Rmem(vfws) = {()})}) ->
				lst ->
...
assume parse_num : lsn -> {vnum | (RmemfstRes(vnum) C Rmem(lsn)) /\
								RobsfstRes (vnum) C Robs (lsn)/\
								RmemsndRes(vnum) C Rmem(lsn)/\
								RobssndRes(vnum) C Robs(lsn)};
