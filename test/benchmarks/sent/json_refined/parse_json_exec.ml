(* 
	Bnf grammar 
	json -> num | str | "{" key : value "}"
	num -> [0-9]+
	str -> [0-9]+
	key -> json 
	value -> json 

	(INFO for files in the directory)
	6. (parse_json_exec.ml/spec) (this file, shows grammar for the parser and a bunch of tests for the json parser and info about other files.)
	
	1. (json_with_ws.ml/spec) (verifies the implementation for parse_spaces)
	2. (json_num_with_token.ml/spec) (implements a higher-order function parse_token and uses this to implement a number parser with possible ws chars)
	3. (json_colon_imprecise.ml/spec) (uses the same parse_token function to build number and colon parsers, the specifications can be made more precise) 
	4. (json_colon_precise.ml/spec) (precise spec for parse_colon, and limitations)
	5. (json_json_token.ml/spec) (full json using token, keeping the limitation in mind)
...
let () = 
	...
	let res4 = parse_json_ref test4 in
 	...
 	(*a*)
 	let test6 = [97] in 
	let res6 = parse_json_ref test6 in
 	
 	(*__a*)
 	let test7 = [32;32;97] in 
	let res7 = parse_json_ref test7 in
...
let () = Printf.printf "%s" ("\n test6 ") in 
    let () = Printf.printf "%s" ("\ninput list "^(intlist_to_string test6)) in 
    let () = Printf.printf "%s" ("\n output "^(output_to_string res6)) in  
    
    let () = Printf.printf "%s" ("\n test7 ") in 
    let () = Printf.printf "%s" ("\ninput list "^(intlist_to_string test7)) in 
    let () = Printf.printf "%s" ("\n output "^(output_to_string res7)) in  
