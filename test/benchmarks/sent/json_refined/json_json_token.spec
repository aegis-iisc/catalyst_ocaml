...
parse_field_ref : rfls -> {vr| RmemfstRes(vr) C= Rmem (rfls)  /\
								     RobsfstRes (vr) C= Robs (rfls) /\
								     RmemsndRes(vr) C= Rmem(rfls)/\
								     RobssndRes(vr) C= Robs(rfls)};


parse_json : inp ->  {outpair | ( RmemfstRes(outpair) C= Rmem (inp) ) /\
...
parse_json_ref : rinlist -> {routpair | ( RmemfstRes(routpair) C= Rmem (rinlist) ) /\
