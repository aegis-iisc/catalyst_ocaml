...
assume parse_num : inp -> {vnum | (RmemfstRes(vnum) C= Rmem(inp)) /\
								RobsfstRes (vnum) C= Robs (inp)/\
								RmemsndRes(vnum) C= Rmem(inp)/\
								RobssndRes(vnum) C= Robs(inp)};

assume parse_num_ref : lnum -> {vlnum | RmemfstRes(vlnum) C= Rmem(lnum) /\
